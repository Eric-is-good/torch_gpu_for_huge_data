...
def BIGmm(mat1, mat2, Slice_size, thread_num=6, gpu_ids=[0]):
    """
    ...
    with ThreadPoolExecutor(max_workers=thread_num) as pool:
        for i in range(shape1[0] // Slice_size[0]):
            for j in range(shape2[1] // Slice_size[1]):
                pool.submit(meta_mm, a_index, b_index, Slice_size[0], Slice_size[1], random.choice(gpu_ids))
        ...
        if shape1[0] % Slice_size[0] != 0 or shape2[1] % Slice_size[1] != 0:
            ...
            for i in range(shape2[1] // Slice_size[1]):
                pool.submit(meta_mm, a_index, b_index, shape1[0] - a_index, Slice_size[1], random.choice(gpu_ids))
                b_index += Slice_size[1]

            a_index = 0
            for i in range(shape1[0] // Slice_size[0]):
                pool.submit(meta_mm, a_index, b_index, Slice_size[0], shape2[1] - b_index, random.choice(gpu_ids))
                a_index += Slice_size[0]

            pool.submit(meta_mm, a_index, b_index, shape1[0] - a_index, shape2[1] - b_index, random.choice(gpu_ids))
